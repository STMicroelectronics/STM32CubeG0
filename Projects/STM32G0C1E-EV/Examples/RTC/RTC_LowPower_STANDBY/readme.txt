...
In the associated software, the system clock is set to 64 MHz, the SysTick is 
...
In case of earlier wakeup, by reset button for instance, LED3 and LED1 are On.

One of the above scenario can occur.
 - LED1 is turned ON when RTC Alarm occurs.
 - LED3 toggling: HAL configuration failed (system will go to an infinite loop)
 - LED_ORANGE ON: After wake-up, SW did not detect a day elapsed
 - LED_GREEN & LED_ORANGE ON: day elapsed well detected but date update failed
 - LED_GREEN & LED_BLUE ON: date update performed correctly
